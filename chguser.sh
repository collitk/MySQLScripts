...
clhelp () {
  cmd=$(basename $0)
  echo "Syntax $cmd" >&2
  echo "$cmd hostname chguser scriptfile {\$option} (script file may contain \$username, \$hostname \$option and it will be expanded before execution in mysql \$option being optional" >&2
  echo "$cmd hostname chguser database   (grants read only on database)" >&2
  echo >&2
...
chg_user=$(echo $2 | tr -dc '[:alnum:]_-' | tr '[:upper:]' '[:lower:]')
if [[ -z $chg_user ]] ; then
  clhelp
  exit 2
fi
# Read the new username from the command line
#read -p "Enter the new username: " chg_user

# Generate a random password for the new user
chg_user_password=$(generate_random_password 12)

# Create a new MySQL user with the random password
create_user_query="ALTER USER $chg_user@'%' IDENTIFIED BY '$chg_user_password';"
...
if [ $# -ge 3 ]; then
    ...
    if [ -r $3 ]; then
       ...
       echo "GRANT SELECT ON $3.* TO $chg_user@'%';" | mysql -h ${hostname}
    fi
fi

    grant_user_output=$(echo "show grants for $chg_user;" | mysql -h ${hostname})

# Print the hostname, username, and password
echo "Scriptname: $script_file"
echo "Hostname:   $hostname"
echo "Username:   $chg_user"
echo "Password:   $chg_user_password"
